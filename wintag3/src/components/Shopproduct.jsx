...
export default function ShopItem() {
  ...
  const { id } = useParams();
  const Currentpro = shop_product.find((p) => p.id === Number(id));
  if (!Currentpro) {
    return (
      <>
  ...
  const Relatedproducts = shop_product.filter((p) => p.id !== Number(id));
  return(
  <>
    <div className="container-fluid px-lg-5 py-5">
          ...
          <div className="col-lg-6 d-flex flex-column justify-content-center">
            <div className="ps-lg-3">
              <span className="badge bg-light text-dark border mb-3 px-3 py-2 text-uppercase tracking-wider fw-semibold">
                ...
                <h4 className="text-muted">Brand : {Currentpro.brand}</h4>
              ...
              <div className="d-flex flex-column flex-sm-row gap-3 pt-2">
                <button className="btn btn-dark btn-lg flex-fill py-3 rounded-pill fw-semibold shadow-sm transition-all" onClick={()=> addTocart(Currentpro)}>
      ...
      <div>
        <h1 className="p-5 display-3">You might also like this</h1>
        <Swiper
        slidesPerView={6}
        spaceBetween={30}
        modules={[Navigation]}
        ...
        {Relatedproducts.map((item) => (
          <SwiperSlide key={item.id}>
            <div className=" h-100 text-center position-relative">
              <div className="product-image position-relative overflow-hidden rounded-3 bg-light">
                <img
                  src={item.image}
                  alt={item.name}
